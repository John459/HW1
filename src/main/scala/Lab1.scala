...
object Lab1 extends jsy.util.JsyApplication {
  ...
  def repOk(t: SearchTree): Boolean = t match {
    case Empty => true
    case Node (l: SearchTree, d: Int, r: SearchTree) =>
      (l,r) match {
        case (Empty, Empty) => true
        case (Node(ll: SearchTree, lValue: Int, rr: SearchTree), Empty) =>
          if (lValue >= d) false else true
        case (Empty, Node(ll: SearchTree, rValue: Int, rr: SearchTree)) =>
          if (rValue < d) false else true
        case _ => repOk(l) && repOk(r)
      }
  }
